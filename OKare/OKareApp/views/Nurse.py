...
#list all help requests that the user is allowed to accept
@login_required
@user_passes_test(is_nurse)
def list_all_help_requests(request):
    account = request.user.account
    ...
    return render(request, 'nurse/help_request_overview.html', context)

@login_required
@user_passes_test(is_nurse)
def reload_all_help_requests(request):
    account = request.user.account
    #filter all help request which are no in the same team as the requester, and not those generated by the requester so that the poller
    #only sees help requests in his own team, and those that do not belong to him
...
#list help requests lists all the help requests for a person to accept, for polling
@login_required
@user_passes_test(is_nurse)
def reload_unread_help_requests(request):
    assignTask()
    account = request.user.account
...
@login_required
@user_passes_test(is_nurse)
def reload_opened_help_requests(request):
    account = request.user.account
    try:
        ongoing_task = OngoingTask.objects.get(nurse=account)
...
@login_required
@user_passes_test(is_nurse)
def reload_assigned_help_requests(request):
    account = request.user.account
    try:
